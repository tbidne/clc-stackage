...
`clc-stackage` is based on a `nightly` snapshot. We previously tried just using `nightly`, but we run into build issues a few times a month because e.g. a new package is added that needs to be excluded or have a flag set. We therefore pick an arbitrary `nightly` snapshot that has many packages in it (generally 3000+). When we do want to update the snapshot, there are some general steps we will want to take:
