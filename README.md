...
[![ci](https://github.com/tbidne/clc-stackage/actions/workflows/ci.yaml/badge.svg?branch=snapshot-url)](https://github.com/tbidne/clc-stackage/actions/workflows/ci.yaml)
[![stable](https://github.com/tbidne/clc-stackage/actions/workflows/stable.yaml/badge.svg?branch=snapshot-url)](https://github.com/tbidne/clc-stackage/actions/workflows/stable.yaml)
[![nightly](https://github.com/tbidne/clc-stackage/actions/workflows/nightly.yaml/badge.svg?branch=snapshot-url)](https://github.com/tbidne/clc-stackage/actions/workflows/nightly.yaml)
...
1. Rebase changes, mandated by your proposal, atop the ghc branch (or tag) that corresponds to the snapshot you intend to use. The most natural snapshot choices are either the current stable:

    ```yaml
    # .github/workflows/stable.yaml
    snapshot: 'nightly-<date>'
    ```

    Or just `nightly` for the latest. Ideally we would like to use `nightly`, as it is the most recent and likely easiest GHC to build. But `nightly` can sometimes bring in new, problematic packages, hence we also list a `stable`, as a last known "good" snapshot.
...
While the stable snapshot is more stable than `nightly`, we can still run into problems:

- Hackage revisions can make a build fail e.g. `base<0`.
...
By default, `clc-stackage` queries `https://www.stackage.org/nightly` for snapshot information. In situations where this is not desirable (e.g. the server is not working, or we want to test a custom snapshot), the snapshot can be overridden in two ways.
