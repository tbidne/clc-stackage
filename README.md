...
```
$ ./bin/clc-stackage --batch 50
```

This will split the entire downloaded package set into groups of size 50. Each time a group finishes (success or failure), stdout/err will be updated, and then the next group will start. If the group failed to build and we have `--write-logs save-failures` (the default), then the logs and error output will be in `./output/logs/<pkg>/`, where `<pkg>` is the name of the first package in the group.
...
Thus for optimal performance, you want to take the largest group possible, with the upper limit being no `--batch` argument at all, as that puts all packages in the same group.

One possible workflow is to run `clc-stackage` first with the largest group possible, then run it a second time with a small batch value e.g. `--batch 1`. This will ensure we build the unbroken packages as quickly as possible, while saving the broken packages into the cache, for better output. Of course the quickest option is to skip this second build, and rely on scanning the log fail manually.
