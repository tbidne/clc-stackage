...
setup hLogger modifyPackages = do
  ...
  let pkgs = modifyPackages (responseToPkgs <$> pkgsResponses)

  -- Update dependencies if we have cached results.
  pkgsList <- case cache of
    Nothing -> pure pkgs
    Just oldResults -> do
      oldFailures <-
      ...
      pure $ Set.toList $ Set.union untested oldFailures

  packages <- case pkgsList of
    (p : ps) -> pure (p :| ps)
    [] -> do
...
getResults env = do
  ...
  let currAllTested = Set.union currSuccesses currFailures
      ...
      currUntested = foldl' addUntested Set.empty env.packages
